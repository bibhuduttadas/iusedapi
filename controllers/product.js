...
//console.log('2');
    ...
    User.findOne({
        ...
        if (!err) {
            if (userObj.Status === 0) {
            ...
            } else if (userObj.Name === '' || userObj.Name === null || userObj.Email === '') {
                res.send({
                    "errNum": '1110',
                ...
                requests('http://www.purgomalum.com/service/plain?text=' + req.body.Description, function (error, response, body) {
                    req.body.Description = body;
                    requests('http://www.purgomalum.com/service/plain?text=' + req.body.Name, function (error1, response1, body1) {
                        req.body.Name = body1;
                        ...
                        Products.update({"_id": productId}, updobj, function (err, Obj) {
                            if (!err) {
                                res.send({
                                    ...
                                    "errMsg": 'We appreciate your generosity. Our team is currently reviewing the post and publish it very shortly.',
                                    "errFlag": '0',
                                });
                            }
                        });
...
//    }
    ...
    if (PostedWithin === '1') {
        ...
        StartDate = moment(startdate).subtract(1, 'days').format('YYYY-MM-DD HH:mm:ss');
    } else if (PostedWithin === '7') {
        StartDate = moment(startdate).subtract(7, 'days').format('YYYY-MM-DD HH:mm:ss');
    } else if (PostedWithin === '30') {
        StartDate = moment(startdate).subtract(30, 'days').format('YYYY-MM-DD HH:mm:ss');
    }
    ...
    if (CategoryId === '') {
        ...
        Qry = {'Status': {'$nin': ['2', '3', '1', '4', '5', '6']}, 'PostedBy': {'$ne': UserId}};
        if (LowerPriceRange !== '') {
            if (StartDate !== '') {
                Qry = {'PostedOnGMT': {'$gt': StartDate}, 'Status': {'$nin': ['2', '3', '1', '4', '5', '6']}, 'PostedBy': {'$ne': UserId}, '$and': [{Price: {'$lt': parseFloat(UpperPriceRange)}}, {Price: {'$gte': parseFloat(LowerPriceRange)}}]};
            } else {
                Qry = {'Status': {'$nin': ['2', '3', '1', '4', '5', '6']}, 'PostedBy': {'$ne': UserId}, '$and': [{Price: {'$lt': parseFloat(UpperPriceRange)}}, {Price: {'$gte': parseFloat(LowerPriceRange)}}]};
            }

        } else {
            if (StartDate !== '') {
                Qry = {'PostedOnGMT': {'$gt': StartDate}, 'Status': {'$nin': ['2', '3', '1', '4', '5', '6']}, 'PostedBy': {'$ne': UserId}};
            } else {
                Qry = {'Status': {'$nin': ['2', '3', '1', '4', '5', '6']}, 'PostedBy': {'$ne': UserId}};
            }
        }

        if (SortBy === '3') {//closest first

        } else if (SortBy === '4') {//newest first
        ...
        Qry = {'Status': {'$nin': ['2', '3', '1', '4', '5', '6']}, "CategoryId": CategoryId, 'PostedBy': {'$ne': UserId}};
        if (LowerPriceRange !== '') {
            if (StartDate !== '') {
                Qry = {'PostedOnGMT': {'$gt': StartDate}, 'Status': {'$nin': ['2', '3', '1', '4', '5', '6']}, 'PostedBy': {'$ne': UserId}, "CategoryId": CategoryId, '$and': [{Price: {'$lt': parseFloat(UpperPriceRange)}}, {Price: {'$gte': parseFloat(LowerPriceRange)}}]};
            } else {
                Qry = {'Status': {'$nin': ['2', '3', '1', '4', '5', '6']}, 'PostedBy': {'$ne': UserId}, "CategoryId": CategoryId, '$and': [{Price: {'$lt': parseFloat(UpperPriceRange)}}, {Price: {'$gte': parseFloat(LowerPriceRange)}}]};
            }

        } else {
            if (StartDate !== '') {
                Qry = {'PostedOnGMT': {'$gt': StartDate}, 'Status': {'$nin': ['2', '3', '1', '4', '5', '6']}, 'PostedBy': {'$ne': UserId}, "CategoryId": CategoryId};
            } else {
                Qry = {'Status': {'$nin': ['2', '3', '1', '4', '5', '6']}, 'PostedBy': {'$ne': UserId}, "CategoryId": CategoryId};
            }
        }
//        if (SortBy === '1') {
//
...
//            sortbyQry = {Price: -1};
//        } else
        if (SortBy === '3') {//highest first


        } else if (SortBy === '4') { //distance
        ...
        getUserDetail(UserId, function (err, UserDet) {
            if (UserDet) {
                if (UserDet.Status === 0) {
                    ...
                    if (Longitude === '' || Latitude === '' || typeof Longitude === 'undefined' || typeof Latitude === 'undefined') {
...
//                                    console.log(item.obj);
                                    var mainDistance = geolib.getDistance(
                                            {latitude: item.obj.location.latitude, longitude: item.obj.location.longitude},
...
//                                    console.log(mainDistance);
                                    if (UpperDistance !== '' && UpperPriceRange !== '') {
                                        if ((parseFloat(mainDistance) < parseFloat(UpperDistance) && parseFloat(mainDistance) >= parseFloat(LowerDistance)) && (parseFloat(item.obj.Price) < parseFloat(UpperPriceRange) && parseFloat(item.obj.Price) >= parseFloat(LowerPriceRange))) {
                                            ok = 1;
                                        }
                                    } else if (UpperDistance !== '') {
                                        if (parseFloat(mainDistance) < parseFloat(UpperDistance) && parseFloat(mainDistance) >= parseFloat(LowerDistance)) {
                                            ok = 1;
                                        }
                                    } else if (UpperPriceRange !== '') {
                                        if (parseFloat(item.obj.Price) < parseFloat(UpperPriceRange) && parseFloat(item.obj.Price) >= parseFloat(LowerPriceRange)) {
                                            ok = 1;
                                        }
                                    } else {
                                        ok = 1;
...
//                                        console.log('1');
                                        if (item.obj.Type === '2') {
                                            PurchaseReqeusts.findOne({$query: {"ProductId": item.obj._id.toString(), "Status": {'$ne': "2"}}, $orderby: {Datetime: -1}}, function (err, maxBid) {
                                                if (ii >= skip && ii < limit) {
//                                                    console.log(maxBid);
                                                    if (maxBid) {
        ...
        getUserDetail(UserId, function (err, UserDet) {
            if (UserDet) {
                if (UserDet.Status === 0) { //check user blocked or  not
                    res.send({
                        "errNum": ErrObj.blocked.num,
                        "errMsg": ErrObj.blocked.message,
                        "errFlag": '1'
...
//                    console.log(sortbyQry);
                    var i = 0;
                    var ii = 0;
                    User.find(Qry, {}).sort(sortbyQry).toArray(function (err, orderlistObj) {
                        if (!err) {
                            if (orderlistObj) {
//                                console.log(orderlistObj.slice(0, 10));
                                async.each(orderlistObj, function (item, callbackgetproduct) {
//                                    console.log(item._id.toString());
                                    getProductDetailWithDistance(item._id.toString(), Longitude, Latitude, UpperDistance, LowerDistance, function (err, ItemDetail) {
                                        if (ItemDetail) {
                                            if (ItemDetail.Type === '2') {
                                                PurchaseReqeusts.findOne({$query: {"ProductId": item._id.toString(), "Status": {'$ne': "2"}}, $orderby: {DatetimeGMT: -1}}, function (err, maxBid) {
                                                    if (maxBid) {
...
exports.GetProducts1 = function (req, res) {
//    console.log(req.body);
    var User = db.collection('Products');
    var PurchaseReqeusts = db.collection('PurchaseReqeusts');
    var lastitems = db.collection('lastitems');
...
//                                console.log(result);
                                    async.each(result.results, function (item, callback) {
//                                    console.log(item.obj);
//                                        getUserDetail(item.PostedBy, function (err, UserDetPostedBy) {
                                        var mainDistance = geolib.getDistance(
                                                {latitude: item.obj.location.latitude, longitude: item.obj.location.longitude},
                                                {latitude: Latitude, longitude: Longitude}
                                        ) / 1000;
                                        ...
                                        dta.Price = item.obj.Price.toString();
                                        dta.Currency = item.obj.Currency;
                                        if (typeof UserDet.Photo === 'undefined') {
                                            dta.PostedByImage = '';
                                        } else {
                                            dta.PostedByImage = UserDet.Photo;
                                        }
...
//                                    console.log(mainDistance);
                                        if (UpperDistance === '') {
                                            UpperDistance = userObj.ItemRadius;
                                            LowerDistance = 0;
                                        }
                                        if (UpperDistance !== '') {
                                            if (parseFloat(mainDistance) < parseFloat(UpperDistance) && parseFloat(mainDistance) >= parseFloat(LowerDistance)) {
...
//                                        getUserDetail(item.PostedBy, function (err, UserDetPostedBy) {
                                        ...
                                        var dta = {};
                                        dta.UsedFor = item.UsedFor;
                                        dta.Condition = item.Condition;
                                        dta.ProductId = item._id;
                                        dta.ProductName = item.Name;
                                        dta.PostedBy = item.PostedBy;
                                        dta.PostedByUserName = UserDet.Name;
                                        dta.PostedByPhone = UserDet.Phone;
                                        dta.Distance = mainDistance.toString();
                                        var imgg = item.ImageLinks.split(",");
                                        var MImg = imgg[0];
                                        if (MImg === '' || MImg === null) {
                                            MImg = 'http://iusedapp.com/apis/controllers/default.png';
                                        }
                                        dta.Image = imgg[0];
                                        dta.Price = item.Price.toString();
                                        dta.Currency = item.Currency;
                                        if (typeof UserDet.Photo === 'undefined') {
                                            dta.PostedByImage = '';
                                        } else {
                                            dta.PostedByImage = UserDet.Photo;
                                        ...
                                        dta.QtyRemaining = '0';
                                        dta.Type = item.Type;
                                        dta.Description = item.Description;
                                        var ok = 0;
//                                    console.log(mainDistance);
                                        if (UpperDistance === '') {
                                            UpperDistance = userObj.ItemRadius;
                                        ...
                                        if (UpperDistance !== '' && UpperPriceRange !== '') {
                                            if ((parseFloat(mainDistance) < parseFloat(UpperDistance) && parseFloat(mainDistance) >= parseFloat(LowerDistance)) && (parseFloat(item.Price) < parseFloat(UpperPriceRange) && parseFloat(item.Price) >= parseFloat(LowerPriceRange))) {
                                                ok = 1;
                                            }
                                        } else if (UpperDistance !== '') {
                                            if (parseFloat(mainDistance) < parseFloat(UpperDistance) && parseFloat(mainDistance) >= parseFloat(LowerDistance)) {
                                            ...
                                            if (parseFloat(item.Price) < parseFloat(UpperPriceRange) && parseFloat(item.Price) >= parseFloat(LowerPriceRange)) {
                                                ok = 1;
                                            }
                                        } else {
                                            ok = 1;
...
//                                        console.log('1');
                                            if (item.Type === '2') {
                                                PurchaseReqeusts.findOne({$query: {"ProductId": item._id.toString(), "Status": {'$ne': "2"}}, $orderby: {Datetime: -1}}, function (err, maxBid) {
                                                    if (maxBid) {
...
exports.GetProducts2 = function (req, res) {
//    console.log(req.body);
    var User = db.collection('Products');
    var PurchaseReqeusts = db.collection('PurchaseReqeusts');
    var lastitems = db.collection('lastitems');
...
//                                console.log(result);
                                    async.each(result.results, function (item, callback) {
//                                    console.log(item.obj);
                                        getUserDetail(item.obj.PostedBy, function (err, UserDetPostedBy) {
                                            if(UserDetPostedBy !== null){
                                            //console.log( "posted by : "+item.obj.PostedBy);
                                            var mainDistance = geolib.getDistance(
                                                    {latitude: item.obj.location.latitude, longitude: item.obj.location.longitude},
                                                    {latitude: Latitude, longitude: Longitude}
                                            ) / 1000;
                                            ...
                                            dta.PostedBy = item.obj.PostedBy;
                                            dta.PostedByUserName = UserDetPostedBy.Name;
                                            dta.PostedByPhone = UserDetPostedBy.Phone;
                                            dta.Image = imgg[0];
                                            dta.Price = item.obj.Price.toString();
                                            dta.Currency = item.obj.Currency;
                                            if (typeof UserDetPostedBy.Photo === 'undefined') {
                                                dta.PostedByImage = '';
                                            } else {
                                                dta.PostedByImage = UserDetPostedBy.Photo;
                                            }
...
//        User.find({}, function (err, result) {
    ...
    StartDate = '';
    var Qry = {};
    Qry = {'Status': {'$nin': ['2', '3', '1', '4', '5']}, 'PostedBy': {'$ne': UserId}};
    var type1 = [];
    var type2 = [];
    var type3 = [];
    getUserDetail(UserId, function (err, UserDet) {
...
//                    console.log(limit);
//                    console.log('----------------');
                User.find(Qry, {}).sort().toArray(function (err, orderlistObj) {
                    if (!err) {
                        if (orderlistObj) {
                            ...
                            async.each(orderlistObj, function (item, callbackgetproduct) {
                                ...
                                getProductDetailWithDistance(item._id.toString(), UserDet.location.longitude, UserDet.location.latitude, '', '', function (err, ItemDetail) {
//                                        console.log(2);
                                    if (ItemDetail) {
...
//                                                        console.log(i);
                                                    if (maxBid) {
                                                        ...
                                                        i++;
                                                        ItemDetail.Price = maxBid.Amount;
                                                        MatchWishList(UserDet.WishList, ItemDetail.ProductName, function (err, WishLisst) {
                                                            if (WishLisst) {
...
//    console.log(limit);
    getUserDetail(UserId, function (err, UserDet) {
        ...
        if (UserDet) {
            console.log("UserDetUserDet",UserDet);
            if (UserDet.Status === 0) { //check user blocked or  not
                res.send({
                    "errNum": ErrObj.blocked.num,
                    "errMsg": ErrObj.blocked.message,
                ...
                console.log("UserDetUserDet222");
                var countPro = 0;
                User.find(Qry, {}).sort({PostedOnGMT: -1}).toArray(function (err, orderlistObj) {
                    console.log("errerrerrerrerr",err); 
                    if (!err) {
                        if (orderlistObj) {
                            console.log("orderlistObjorderlistObj"); 
                            async.each(orderlistObj, function (item, callback) {
                                console.log(item._id.toString());
                                getProductDetailWithDistance(item._id.toString(), UserDet.location.longitude, UserDet.location.latitude, '', '', function (err, ItemDetail) {
                                    console.log("ItemDetailItemDetailItemDetail",ItemDetail); 
                                    if (ItemDetail) {
...
//                console.log(1);
                ...
                getUserDetail(UserId, function (err, MainUserDet) {
                    if (MainUserDet) {
                        if (MainUserDet.Status === 0) { //check user blocked or  not
                            res.send({
                                "errNum": ErrObj.blocked.num,
                                "errMsg": ErrObj.blocked.message,
                                "errFlag": '1'
...
//                                    console.log(1);
                                    ...
                                    var OriginalP = '';
                                    var PriceShouldbe = '';
                                    PurchaseReqeusts.findOne({$query: {"ProductId": ProductId.toString(), "Status": {'$ne': "2"}}, $orderby: {Datetime: -1}}, function (err, maxBid) {
                                        if (err) {
                                            ...
                                            res.send({
                                                ...
                                                "errMsg": ErrObj.success.message,
                                                "errFlag": '0',
                                                "Data": [{'Qty': result.Qty, 'OfferPer': result.OfferPer, 'UserName': UserDet.Name,
                                                        'OfferedPrice': OfferedPrice, 'OfferedBy': OfferedBy, 'NextBidShouldBe': PriceShouldbe, 'OriginalPrice': result.Price.toString(),
                                            ...
                                            getUserDetail(maxBid.UserId, function (err, OfferedUser) {
                                                if (OfferedUser) {
                                                    ...
                                                    res.send({
                                                        ...
                                                        "errMsg": ErrObj.success.message,
                                                        "errFlag": '0',
                                                        "Data": [{'Qty': result.Qty, 'OfferPer': result.OfferPer, 'UserName': UserDet.Name, 'OriginalPrice': result.Price.toString(),
                                                                'OfferedPrice': OfferedPrice, 'OfferedBy': OfferedBy, 'NextBidShouldBe': PriceShouldbe,
                                                                'OfferMins': result.OfferMins, 'ExchangeOffer': result.ExchangeOffer, 'PostedOn': result.PostedOn, 'ImageLinks': result.ImageLinks, 'VideoLinks': result.VideoLinks}]
                                                    });
                                                } else {
                                                    res.send({
                ...
                getUserDetail(req.body.UserId, function (err, UserDet) {
                    if (UserDet) {
                        if (UserDet.Status === 0) {
                            ...
                            db.command({
                                geoNear: "Products",
                                near: [parseFloat(UserDet.location.longitude), parseFloat(UserDet.location.latitude)],
                                num: 100000,
                                "query": {'Status': {'$nin': ['2', '3', '1', '4', '5']}, '$or': [{'Name': {$regex: new RegExp(replaced), $options: "i"}}, {'Name': {'$regex': new RegExp(req.body.WishList), $options: "i"}}], 'PostedBy': {'$ne': req.body.UserId}}//'Status': {'$nin': ['2', '3', '1', '4', '5']}, 
                            },
                                    function (err, result) {
//                                        console.log(result);
                                        async.each(result.results, function (item, callbackmeupdatewishlist) {
...
//                                            dta.Distance = item.dis.toString();
                                            dta.Distance = (geolib.getDistance(
                                                    {latitude: item.obj.location.latitude, longitude: item.obj.location.longitude},
                                                    {latitude: UserDet.location.latitude, longitude: UserDet.location.longitude}
                                            ) / 1000).toString();
                                            var MImg = '';
...
// console.log(req.body);
    ...
    var PurchaseReqeusts = db.collection('PurchaseReqeusts');
    var Products = db.collection('Products');
    if (Status !== '1' && Status !== '2' && Status !== '3') {//accepted
        res.send({
            "errNum": '1',
        ...
        PurchaseReqeusts.findOne({"_id": RequestId}, function (err, userObj) {
            if (!err) {
                if (userObj) {
                    if (Status === '3' && Code !== userObj.SecrateCode) {//completed
                        res.send({
                            "errNum": '1',
                            "errMsg": 'Secrate code is not correct ask buyer to generate code again',
                            "errFlag": '1'
                        });
                    } else {
                        var updobj = {};
                        ...
                        PurchaseReqeusts.update({"_id": RequestId}, updobj, function (err, Obj) {
                            if (!err) {
                                ...
                                PurchaseReqeusts.update({'$and': [{"_id": {'$ne': RequestId}}, {"ProductId": userObj.ProductId}]}, {$set: {'Status': '2'}}, {'multi': true}, function (err, Obj) {
                                    if (!err) {
                                        Products.update({"_id": new ObjectID(userObj.ProductId)}, updobj, function (err, Obj) {
                                            if (!err) {
                                                getUserDetail(userObj.UserId, function (err, PushTo) {
                                                    if (PushTo) {
                                                        getUserDetail(userObj.ProductOwner, function (err, Owner) {
                                                            if (Owner) {
                                                                getMainProductDetail(userObj.ProductId, function (err, ProDet) {
                                                                    if (ProDet) {
                                                                        var PushMsg = '';
                                                                        var pushType = '0';
                                                                        if (ProDet.Type === '3') {
//                                                                            console.log('1');
                                                                            pushType = '1';
                                                                            if (Status === '2') {
                                                                                PushMsg = 'Donor of ' + ProDet.Name + ' rejected your request.';
                                                                            } else {
//                                                                                console.log('2');
                                                                                PushMsg = "Congratulations! The donor has accepted your request to donate " + ProDet.Name + ". You can find the donor's contact under 'Donate -> Donor Responses' and schedule the meet accordingly. Thanks";
                                                                                // PushMsg = 'Donor of ' + ProDet.Name + ' may be contacting you shortly. Alternatively, you can contact the donor at  ' + Owner.Phone + '.';
                                                                            }
                                                                        } else {
                                                                            if (Status === '2') {
                                                                                PushMsg = Owner.Name + ', the seller of the ' + ProDet.Name + ' rejected your offer.';
                                                                            } else {
                                                                                PushMsg = Owner.Name + ', the seller of the ' + ProDet.Name + ' almost accepted your offer. He would be contacting you very shortly. Alternatively, you can call the seller at ' + Owner.Phone + '.';
                                                                            }
                                                                        }
                                                                        sendpush(PushTo.DeviceToken, PushMsg, 'iUsed', {'pushType': pushType, 'Type': '1'}, function (err, r) {
                                                                            res.send({
                                                                                "errNum": ErrObj.success.num,
                                                                                "errMsg": ErrObj.success.message,
                                                                                "errFlag": '0',
...
exports.GetCategories = function (req, res) {
    ...
    var Products = db.collection('Products');
    Cats.find({}, {}).sort({count: 1}).toArray(function (err, orderlistObj) {
        if (!err) {
            res.send({
                "errNum": ErrObj.success.num,
                "errMsg": ErrObj.success.message,
...
exports.GenerateCode = function (req, res) {
    ...
    if (ReqeustId === '') {
        ...
        PurchaseReqeusts.update({"_id": new ObjectID(ReqeustId)}, updobjProduct, function (err, Obj) {
            if (!err) {
                res.send({
                    "errNum": ErrObj.success.num,
                    "errMsg": 'Success',
...
//    var OfferTillGMT = req.body.OfferTillGMT;
    ...
    if (OfferTill !== '' || OfferTill !== null) {
        var startdate = moment(GMTTime);
        var OfferTillGMT = moment(startdate).add(OfferTill, 'minutes').format('YYYY-MM-DD HH:mm:ss'); // see the cloning?m YYYY-MM-DD HH:mm:ss
    }
//check product is sold or not and how many qty sold
    var totaltemSold = 0;
    var gterestValue = 0;
    PurchaseReqeusts.find({'ProductId': ProductId}, {}).sort({'DatetimeGMT': 1}).toArray(function (err, orderlistObj) {
        if (!err) {
            var data = [];
            async.each(orderlistObj, function (item, callback) {
//                console.log(item);
...
//                    console.log('1');
                    ...
                    if (item.length >= 5 && item.Type === '1') {
                        res.send({
                            "errNum": ErrObj.unexpected_error.num,
                            "errMsg": "Oops, this item got more than 5 reqeusts so you can try again later.",
                            "errFlag": '1'
                        });
                    } else {
                        if (item.Status === '3') {
...
//                                            console.log('4');
                                        ...
                                        var currency = '';
                                        if (result.Currency === '' || result.Currency === null || result.Currency === 'null') {
                                        ...
                                        if (result.Type === '1') {
                                            ...
                                            if (exists) {
                                                PurchaseReqeusts.remove({'$and': [{"ProductId": ProductId}, {'UserId': UserId}]}, function (err, exists) {
                                                    sendReqeust(result.PostedBy, result.Type, qty, availableQty, currency);
                                                });
                                            } else {
                                                PurchaseReqeusts.count({'$and': [{"ProductId": ProductId}, {"Status": '0'}]}, {}, function (err, totalReq) {
                                                    if (totalReq + 1 <= 5) {
                                                        if (totalReq + 1 === 5) {
                                                            Products.update({"_id": new ObjectID(ProductId)}, {$set: {'Status': '6'}}, function (err, Obj) { //unpublished
                                                                sendReqeust(result.PostedBy, result.Type, qty, availableQty, currency);
                                                            });
                                                        } else {
                                                            sendReqeust(result.PostedBy, result.Type, qty, availableQty, currency);
                                                        ...
                                                        res.send({
                                                            ...
                                                            "errMsg": 'Sorry, the seller is no more accepting the offers at this time. ',
                                                            "errFlag": '1'
                                                        });
                                                    }
                                        ...
                                        if (result.Type === '2') {
                                            if (Amount === '') {
                                                ...
                                                PriceRange.findOne({"$and": [{'From': {'$lte': result.Price}}, {'To': {'$gt': result.Price}}]}, function (err1, dataexists) {
                                                    if (!err1)
                                                    {
                                                        if (dataexists) {
...
//                                                        console.log(perMax);
                                                        ...
                                                        if (gterestValue === 0 || gterestValue === '0') {
                                                            gterestValue = result.Price;
                                                        }
                                                        var OriginalP = parseInt(gterestValue);
//                                                        console.log('OriginalP : ' + OriginalP);
                                                        var PriceShouldbe = parseInt(OriginalP + ((OriginalP * per) / 100));
//                                                        console.log('PriceShouldbe : ' + PriceShouldbe);
                                                        var PriceShouldbelessthen = parseInt(OriginalP + ((OriginalP * perMax) / 100));
//                                                        console.log('PriceShouldbelessthen : ' + PriceShouldbelessthen);
                                                        if (PriceShouldbe > parseInt(Amount)) {
                                                            res.send({
                                                                ...
                                                                'errMsg': 'Hey, We have certain price considerations based on the competition. Your offer must be at least ' + currency + '' + PriceShouldbe,
                                                                "errFlag": '1'
                                                            });
                                                        } else if (PriceShouldbelessthen < parseInt(Amount)) {
                                                            res.send({
                                                                "errNum": ErrObj.faild.num,
                                                                "errMsg": 'Hey, We have certain price considerations based on the competition. You offer has to be between ' + currency + '' + PriceShouldbe + ' and ' + currency + '' + PriceShouldbelessthen + ' at this time. Thank you. ',
                                                                "errFlag": '1'
                                                            });
                                                            //
                                                        } else {
            ...
            if (UserDet) {
                if (UserDet.Status === 0) { //check user blocked or  not
                    ...
                    if (type === '3') {
                        ...
                        updobj = {'ProductId': ProductId, 'Status': '0', 'Message': Message, 'ProductOwner': owner, 'Qty': qty, 'OfferTillGMT': OfferTillGMT, 'UserId': UserId, 'Datetime': Datetime, 'DatetimeGMT': GMTTime, 'OfferTill': OfferTill, 'Amount': Amount, 'appliedOffer': appliedOffer,
                            'BuyerLocation': UserDet.FullAddress, 'BuyerLatitude': UserDet.location.latitude, 'BuyerLongitude': UserDet.location.longitude, 'Currency': currency};
...
//                                } else {
                                getUserDetail(owner, function (err, OwnerDet) {
                                    if (OwnerDet) {
                                        ...
                                        var PushMsg = 'You have some request(s) from the needy.. Some people are interested. Check the details now!';
                                        sendpush(OwnerDet.DeviceToken, PushMsg, 'iUsed', {'pushType': '1', 'Type': '0'}, function (err, r) {
//                                            console.log('2');
                                            res.send({
                                                "errNum": ErrObj.success.num,
                                                "errMsg": 'Thanks for the message. We share it with the donor and update you on the status shortly. Once your request is approved by the donor, you are free to contact the donor directly.',
                                                "errFlag": '0'
                                            });
                                        });
                                    } else {
...
//                                }
                        ...
                        updobj = {'ProductId': ProductId, 'Status': '0', 'Message': Message, 'ProductOwner': owner, 'Qty': qty, 'OfferTillGMT': OfferTillGMT, 'UserId': UserId, 'Datetime': Datetime, 'DatetimeGMT': GMTTime, 'OfferTill': OfferTill, 'Amount': Amount, 'appliedOffer': appliedOffer,
                            'BuyerLocation': UserDet.FullAddress, 'BuyerLatitude': UserDet.location.latitude, 'BuyerLongitude': UserDet.location.longitude};
                        PurchaseReqeusts.insert(updobj, function (err, result) {
                            if (!err) {
                                    ...
                                    if (!err) {
                                        getUserDetail(owner, function (err, OwnerDet) {
                                            if (OwnerDet) {
                                                ...
                                                sendpush(OwnerDet.DeviceToken, 'You got some offer from the buyer(s). Please check and respond quickly.', 'iUsed', {'pushType': '0', 'Type': '0'}, function (err, r) {
                                                    if (orderlistObj.toString() === '5') {
                                                        Products.update({"_id": new ObjectID(ProductId)}, {$set: {'Status': '2'}}, function (err, Obj) { //unpublished
                                                            if (!err) {
                                                                res.send({
                                                                    "errNum": ErrObj.success.num,
                                                                    "errMsg": 'Thanks for the offer. We will get back to you when seller accepts your offer.',
                                                                    "errFlag": '0'
                                                                });
                                                            }
                                                        });
    ...
    getUserDetail(UserId, function (err, UserDet) {
        ...
        if (UserDet) {
            if (UserDet.Status === 0) { //check user blocked or  not
                ...
                PurchaseReqeusts.find({'UserId': UserId}, {}).sort().toArray(function (err, PurchaseReqeust) {
                    if (!err) {
                        var MyRequestsTobuy = [];
                        async.each(PurchaseReqeust, function (item, callback) {
                            if (item) {
                                ...
                                getUserDetail(otherUserId, function (err, UserDet) {
                                    var mainData = {};
                                    if (UserDet) {
                                        getProductDetail(item.ProductId, For, function (err, ProDet) {
                                            if (ProDet) {
                                                ...
                                                if (ProDet.ImageLinks !== '' && ProDet.ImageLinks !== null && typeof ProDet.ImageLinks !== 'undefined') {
                                                    mainData.ProductImage = ProDet.ImageLinks.split(",")[0];
                                                } else {
                        ...
                        , function (err) {
                            ...
                            Products.find({'$and': [{'PostedBy': UserId}, {'Type': For}]}, {}).sort({'PostedOnGMT': -1}).toArray(function (err, MyProducts) {
                                if (!err) {
                                    var MyRequestsToSell = [];
                                    async.each(MyProducts, function (ProDet, callbackMain) {
                                        var check = 0;
                                        PurchaseReqeusts.find({'ProductId': ProDet._id.toString()}, {}).sort({'OfferTillGMT': -1}).toArray(function (err, PurchaseReqeust) {
                                            if (!err) {
                                                async.each(PurchaseReqeust, function (item, callback) {
                                                    if (item) {
                                                        ...
                                                        getUserDetail(otherUserId, function (err, UserDet) {
                                                            ...
                                                            if (UserDet) {
                                                                ...
                                                                SellReqs.OfferApplied = item.appliedOffer;
                                                                ...
                                                                SellReqs.expiresOn = moment(item.Datetime).add(item.OfferTill, 'minutes').format('YYYY-MM-DD HH:mm:ss');
                                                                var expiresOnvar = moment(item.DatetimeGMT).add(item.OfferTill, 'minutes').format('YYYY-MM-DD HH:mm:ss');
                                                                var ms = moment(expiresOnvar, 'YYYY-MM-DD HH:mm:ss').diff(moment(GMTTime, 'YYYY-MM-DD HH:mm:ss'));
                                                                ...
                                                                mainData.UsedFor = ProDet.UsedFor.toString();
                                                                mainData.Condition = ProDet.Condition;
                                                                SellReqs.ImotionalMessage = item.Message;
                                                                ...
                                                                mainData.Status = item.Status;
                                                                if (typeof in_array(MyRequestsToSell, item.ProductId) === 'undefined') {
                                                                    if (SellReqs.Status === '0' || SellReqs.Status === '1') {
                                                                        mainData.Reqeusts.push(SellReqs);
                                                                    }
                                                                    mainData.count = mainData.Reqeusts.length;
                                                                    MyRequestsToSell.push(mainData);
                                                                } else {
                                                                    if (SellReqs.Status === '0' || SellReqs.Status === '1') {
                                                                        var index = in_array(MyRequestsToSell, item.ProductId);
//                                                                        console.log(MyRequestsToSell[index].Reqeusts.length);
                                                                        if (mainData.Type === '2' && MyRequestsToSell[index].Reqeusts.length < 5) {
                                                                            MyRequestsToSell[index].Reqeusts.push(SellReqs);
                                                                            var ct = parseFloat(MyRequestsToSell[index].count) + 1;
                                                                            MyRequestsToSell[index].count = ct.toString();
                                                                        } else if (mainData.Type === '3' && MyRequestsToSell[index].Reqeusts.length < 3) {
                                                                            MyRequestsToSell[index].Reqeusts.push(SellReqs);
                                                                            var ct = parseFloat(MyRequestsToSell[index].count) + 1;
                                                                            MyRequestsToSell[index].count = ct.toString();
                                                    ...
                                                    if (check === 0) {
                                                        ...
                                                        mainData.ProductName = ProDet.Name;
                                                        if (ProDet.ImageLinks !== '' || ProDet.ImageLinks !== null) {
                                                            mainData.ProductImage = ProDet.ImageLinks.split(",")[0];
                                                        } else {
                                                            mainData.ProductImage = '';
                                                        }
...
function getProductDetail(ProductId, For, callbackProDet) {
    ...
    customer.findOne({'$and': [{"_id": _id}, {'Type': For}]}, function (err, result1) {
        if (!err) {
...
//if order accept getting orderdata
function getProductDetailWithDistance(ProductId, long, lat, upDis, lowDis, callbackprodewithdist) {
    var customer = db.collection('Products');
    var Terms = db.collection('Terms');
    var PurchaseReqeusts = db.collection('PurchaseReqeusts');
    var _id = new ObjectID(ProductId);
...
//                                            dta.Distance = item.dis.toString();
                        ...
                        if (!err) {
                            if (upDis !== '') {
                                if (parseFloat(dta.Distance) < parseFloat(upDis) && parseFloat(dta.Distance) >= parseFloat(lowDis)) {
...
//        }
//    });
    sendpush('erbtO5LHZRo:APA91bHoN80rV-eqJjBEb-76ETcbCJXGPSElnxJ3rz9p1mpuxc8He3TuNw-wznapGeH99d68tyStc18zre4WUySMhFSsmDGSa_bueWa6dMFnv4nFg2bPhPmRWV1-pFIODUY81osGh4U6', 'kya be lodu?', 'iUsed', {'pushType': '0', 'Type': '0'}, function (err, r) {
        if (!err) {
            res.send({
                "errFlag": r
            });
...
exports.GetItemConditions = function (req, res) {
    res.send({
        "errFlag": '0',
        'Conditions': ['Brand-new / Unused', 'Almost looking new', 'Well maintained', 'Refurbished', 'Few scratches',
            'Little broken', 'Some repair work needed', 'Not working']
