...
class MongoDumper {
    ...
    public function run($database, $debug = false) {
        ...
        try {
            ...
            $this->current_dump_path = '/var/www/html/db/';
...
try {
    $command = "mongodump --db market";
    $results = shell_exec('export LC_ALL=C');
    $results = shell_exec($command);
//    print_r($results);
//    $dumper = new MongoDumper("/var/www/html/db/");
//    $dumper->run("market", true);
} catch (Exception $ex) {
    print_r($ex);
}
